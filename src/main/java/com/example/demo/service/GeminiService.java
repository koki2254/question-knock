    ...
    public String askGemini(String questionText, String userMessage, String imageBase64, String mimeType) {
        ...
        try {
            ...
            String prompt = String.format("""
                ...
                【構成ルール】
                回答は必ず以下の2部構成とし、その間に「---」を入れてください。
                1. 概要（2〜3行で結論を簡潔に）
                2. 詳細（具体的な計算過程や理由を詳しく）
                
                【表記ルール（厳守）】
                1. 文字の強調（「*」や「$」、「#」、「**」など）は一切使用しないでください。
                2. 数式や四則演算を記述する際は、以下の全角記号を優先して使い、読みやすく記述してください。
                   足し算：＋
                   引き算：－
                   掛け算：×
                   割り算：÷
                ...
                "【注意】問題文の中に図や式（BNF定義など）が足りない場合、添付された画像の中にその情報が含まれています。
                画像の内容を最優先で解析して回答してください。"
